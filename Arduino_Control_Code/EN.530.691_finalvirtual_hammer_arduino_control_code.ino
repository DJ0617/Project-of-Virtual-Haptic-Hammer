//-------------------------------------------------------------------------------------------//
// EN.530.691 Haptic Interface Design 
// Final Project of virtual haptic hammer of Team 3
// Our team is made up of Daijie Bao, Mengxu Xie, and Zihao Lin
// Code Template from the unground haptik project of Naveed Riaziat and Alexandra Miller 
// Code Template location: https://github.com/nriaziat/Ungrounded_Hapkit
// Revised by Daijie Bao 
//-------------------------------------------------------------------------------------------//
// Includes
#include <math.h>
#include <SPI.h>
...
//---------- Add for vibration motor control of Team 3 Final Project ----------------------------//
int pwmPin_2 = 6; // PWM output pin for motor 2 (vibration motor)
int dirPin_2 = 7; // direction output pin for motor 2 (vibration motor)
//-----------------------------------------------------------------------------------------------//

int sensorPosPin = A2; // input pin for MR sensor
int fsrPin = A3; // input pin for FSR sensor
//------------------- Need Change for Team 3 Final Project -----------------------------------//
...
//---------- Add for vibration motor control of Team 3 Final Project ----------------------------//
// Parameters for virtual nail position and stiffness of the virtual nail (Change the numbers during Test for final demo)
double x_nail = 0.005; // position of the virtual nail [m]
double k_nail = 350.0; // stiffness of the virtual nail [N/m]
//-----------------------------------------------------------------------------------------------//
...
//---------- Add for vibration motor control of Team 3 Final Project ----------------------------//
// Force output variable for vibration motor 
double force_2 = 0;                 // Force at the handle to render knock nail 
double Tp_2 = 0;                    // Torque of the motor pulley for vibration motor control 
double duty_2 = 0;                  // Duty cylce (between 0 and 255) for vibration motor control 
unsigned int output_2 = 0;          // Output command to the vibration motor control 
//-----------------------------------------------------------------------------------------------//
...
void setup()
{
  ...
  //---------- Add for vibration motor control of Team 3 Final Project ----------------------------//
  // Set PWM frequency for vibration motor 
  ...
  //---------- Add for vibration motor control of Team 3 Final Project ----------------------------//
  // Output Pins for Vibration motor 
  pinMode(pwmPin_2, OUTPUT); // PWM pin for vibration Motor 
  pinMode(dirPin_2, OUTPUT); // dir pin for vibration Motor 
  //-----------------------------------------------------------------------------------------------//
  ...
  analogWrite(pwmPin_2, 0); // set to not be spinning (0/255) for vibration motor
  digitalWrite(dirPin_2, LOW); // set direction for vibration motor   
  //-----------------------------------------------------------------------------------------------//
...
void loop()
{
 ...
 //------------------- Need Change for Team 3 Final Project -----------------------------------//  
  ...
  //Serial.println(xh,5); // Use for Team 3 Project Calibration
 //---------------------------------------------------------------------------------------------//
 ...
 // ------------------------- Changed for Team 3 Final Project -----------------------------------------------//
  ...
  if (theta_s > x_msd && xh > x_nail)
  {
    ...
    force_2 = -k_nail * (xh - x_nail); // equation to render the virbration feedback of when hammer knocking nail
    ...
    // to do: what is the force when the xh < x_msd
    force = 0;
    force_2 = 0; // force for render the vibration feedback of the situation when hammer not touch nail
...
//using this counter is optional, just limits how many times we print to Processing (increasing baud rate can also help if need be)
 ...
 // ------------------------------- Add for vibration motor torque control of Team 3 final Project -----------------------------------------//
 Tp_2 = ((rp * rh) / rs) * force_2 ; // Compute the require motor pulley torque (Tp) to generate that force using kinematics
 ...
 // ----------------------------------------------------------------------------------------------------------------------------------------//
  ...
  //} else if (duty_2 < 0) { 
    //duty_2 = 0;
  //}  
  //output_2 = (int)(duty_2* 255);   // convert duty cycle to output signal for vibration motor control
  //analogWrite(pwmPin_2, output_2); // output the signal for vibration motor control 
//}
  // ------------------------------------------------------------------------------------------------------//
